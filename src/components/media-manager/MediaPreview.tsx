...
interface MediaPreviewProps {
  ...
  onUpdate?: (item: ExtendedMediaItem, updates: Partial<ExtendedMediaItem>) => Promise<void>;
}

export function MediaPreview({ item, onClose, onDelete, onUpdate }: MediaPreviewProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error("Failed to update media information");
      console.error(error);
    } finally {
      setIsSaving(false);
    }
        ...
        <div className="flex flex-col md:flex-row h-full max-h-[90vh] overflow-y-auto">
          ...
          <div className="bg-muted/30 flex items-center justify-center p-8 md:w-2/3">
            {isImage ? (
              <div className="relative w-full max-h-[60vh] aspect-square">
                <Image
                  src={fileUrl}
                  alt={isEditing ? editedData.altText || "" : item.altText || item.originalName}
                  fill
              ...
              <div className="text-center">
                ...
                <p className="mt-3 text-sm text-muted-foreground">{item.mimeType}</p>
              </div>
            )}
          </div>
          ...
          <div className="p-6 border-t md:border-t-0 md:border-l md:w-1/3 space-y-4 overflow-y-auto max-h-[90vh]">
            ...
            <div className="flex justify-between items-start gap-2">
              <h3 className="font-semibold text-lg truncate flex-1">{item.originalName}</h3>
                        ...
                        <div className="space-y-2">
                          ...
                          {isEditing ? (
                            ...
                            <div className="text-sm text-muted-foreground">
                              {item.altText ? (
                                <p className="whitespace-pre-wrap">{item.altText}</p>
                              ) : (
                                <p className="italic">No alternative text provided</p>
                              )}
            ...
            <div className="space-y-3 pt-4">
              <div className="flex gap-2">
                <Button variant="outline" size="default" className="flex-1" onClick={copyUrl}>
                  {copied ? <Check className="w-4 h-4 mr-2" /> : <Copy className="w-4 h-4 mr-2" />}
                  {copied ? "Copied" : "Copy URL"}
                </Button>
                <Button variant="outline" size="default" className="flex-1" onClick={downloadFile}>
